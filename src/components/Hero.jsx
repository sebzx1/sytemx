...
const Hero = () => {
  ...
  useEffect(()=>
  {
    if (textRef.current && !isInitialized.current) {
      // Destruir completamente cualquier instancia previa
      if (textRef.current._ityped) {
        textRef.current._ityped.destroy();
        textRef.current._ityped = null;
      }
      ...
      init(textRef.current, { 
        showCursor: true, 
        strings: ['Desarrollador Web', 'Tecnico Sistema', 'Aprendiz', 'Desarrollador Software'],
        loop: true,
        typeSpeed: 100,
        backSpeed: 50,
        startDelay: 500
    ...
    return () => {
      if (textRef.current && textRef.current._ityped) {
        textRef.current._ityped.destroy();
        textRef.current._ityped = null;
        textRef.current.innerHTML = '';
