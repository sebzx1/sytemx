...
require "config/conexion.php";  /*invoca el archivo php */
...
<main id="principal">
    <form action="inicio.php" method="post">
        ...
        <div class="inputGroup inputGroup1">
            ...
            <input type="email" name="correo" id="loginEmail" maxlength="254" />
        </div>
        ...
        <div class="inputGroup inputGroup3">
            <button type="submit" name="btnIngresar" id="login" class="btn btn-primary btn-lg">Iniciar Secion</button>

        </div>

        <div className="d-flex justify-content-end" style={{ marginTop: '10px' }}>
            <a href="http://localhost:5173/systemx" className="btn btn-primary">
...
<script>
  particlesJS(
    ...
    $correo = $_POST['correo']; /*invocando el imput correo y convirtiendo a variable*/
    $clave = $_POST['clave']; /*invocando el imput clave y convirtiendo a variable*/

    if (isset($_POST['btnIngresar'])) { /*si presional el boton ingresar entonces*/
        //consulta al SQL
//select * from busca en la base de datos usuarios
//busca la tabla usuario 
//busaca la tabla clave 
    
        $sql = "SELECT u.correo,u.clave,u.nombres,u.codigo,u.cargo
                        FROM usuario u
                        WHERE u.correo = '$correo' AND u.clave = '$clave' LIMIT 1";

        $ejecutar = $conexion->query($sql);

        $row = mysqli_fetch_object($ejecutar);

        if ($correo == $row->correo && $clave == $row->clave) { /*verificar si el usuario y clave estan en la bd*/
            $_SESSION['nombres'] = $row->nombres;
            $_SESSION['usuario'] = $row->codigo;
            $_SESSION['cargo'] = $row->cargo;

            header('location:index.php'); /*abrir el archivo index php*/

        } else { /*si el usuario y la contraseña no son correptos*/
            echo '<div class="mensajeContra"> Usuario y/o contraseña incorrectos </div>';
            echo '<div class="mensajeContra"> vuelve a ingresar los datos </div>';
        }
